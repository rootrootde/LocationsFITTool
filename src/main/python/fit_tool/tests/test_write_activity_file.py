...
class TestWriteActivityFile(unittest.TestCase):
    ...
    def test_write_activity_file_iteratively(self):
        ...
        with open(out_path, "wb") as file_object:
            ...
            write_message(
                definition
            )  # write the definition only once for all the record messages
            write_message(message)
    ...
    def test_write_activity_with_developer_data_fields(self):
        ...
        dev_field = DeveloperField(
            developer_data_index=developer_data_index, field_id=0, size=1
        )
        ...
        with open(out_path, "rb") as file_object:
            ...
            for record in fit_file.records:
                ...
                if isinstance(message, RecordMessage):
                    self.assertEqual(
                        message.developer_fields[0].name, doughnuts_earned_field_name
                    )
                    self.assertEqual(message.developer_fields[0].get_value(0), 5)
                    self.assertEqual(
                        message.developer_fields[0].units, doughnuts_earned_field_units
                    )
