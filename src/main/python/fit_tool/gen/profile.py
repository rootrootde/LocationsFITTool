    ...
    def load(cls, filename):
        ...
        for index, row in enumerate(ws.rows):
            ...
            if type_name:
                ...
                if not type_base_type:
                    ...
                    print("Warning: Unknown base_type {}".format(type_base_type))
                    continue
        ...
        for index, row in enumerate(ws.rows):
            ...
            if message_name:
                ...
                # it.
                message_id = profile.get_type_by_name("mesg_num").values_by_name[
                    message_name
                ]
                ...
                if raw_ref_field_names:
                    ref_field_names = [
                        name.strip() for name in raw_ref_field_names.split(",")
                    ]

                ref_field_values = []
                if raw_ref_field_values:
                    ref_field_values = [
                        value.strip() for value in raw_ref_field_values.split(",")
                    ]

                if (
                    field_name == "custom_target_power_high"
                    or field_name == "custom_target_power_low"
                ):
                ...
                if (
                    ...
                    or field_name == "custom_target_heart_rate_low"
                ):
                    ref_field_names.append("target_type")
                ...
                if (
                    ...
                    or field_name == "custom_target_speed_low"
                ):
                    ref_field_names.append("target_type")
                ...
                for subfield in field.sub_fields:
                    if subfield.ref_field_map:
                        ...
                        for key, values in subfield.ref_field_map.items():
                            ...
                            resolved_values = []
                            for item in values:
                                resolved_value = profile.get_type_by_name(
                                    field.type_name
                                ).get_value_by_name(item)
