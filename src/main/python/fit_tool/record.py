...
class RecordHeader:
    ...
    def from_message(cls, message: Message):
        if isinstance(message, DefinitionMessage):
            ...
            return cls(
                has_developer_fields=has_developer_fields, local_id=message.local_id
            )
        else:
    ...
    def from_bytes(cls, bytes_buffer: bytes, offset: int = 0):
        ...
        is_time_compressed = (
            byte & cls.IS_TIME_COMPRESSED_BIT_MASK
        ) == cls.IS_TIME_COMPRESSED_BIT_MASK
        if is_time_compressed:
            ...
            is_definition = (
                byte & cls.IS_DEFINITION_BIT_MASK
            ) == cls.IS_DEFINITION_BIT_MASK
            has_developer_fields = (
                byte & cls.HAS_DEVELOPER_FIELDS_BIT_MASK
            ) == cls.HAS_DEVELOPER_FIELDS_BIT_MASK
    ...
    def to_bytes(self):
        ...
        if self.is_time_compressed:
            ...
            byte |= (
                self.local_id << 5
            ) & RecordHeader.TIME_COMPRESSED_LOCAL_ID_BIT_MASK
    ...
    def from_bytes(
        ...
        if header.is_definition:
            ...
            message = DataMessage.from_bytes(
                definition_message, developer_fields, bytes_buffer, offset=offset
            )
