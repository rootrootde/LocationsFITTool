...
class DefinitionMessage(Message):
    def __init__(
        ...
        super().__init__(
            local_id=local_id,
            global_id=global_id,
            size=DefinitionMessage.calculate_size(
                field_definitions, developer_field_definitions
            ),
            endian=endian,
        )
        ...
        self.developer_field_definitions = (
            developer_field_definitions if developer_field_definitions else []
        )
    ...
    def remove_field(self, field_id: int):
        ...
        if field_definition:
            ...
            self.size = DefinitionMessage.calculate_size(
                self.field_definitions, self.developer_field_definitions
            )

    def remove_developer_field(self, developer_data_index: int, field_id: int):
        field_definition = self.get_developer_field_definition(
            developer_data_index, field_id
        )
        if field_definition:
    ...
    def get_developer_field_definition(
        ...
        return next(
            (
                ...
                if x.developer_data_index == developer_data_index
                and x.field_id == field_id
            ),
            None,
        )
    ...
    def to_row(self) -> list:
        ...
        for field_definition in self.field_definitions:
            ...
            if not field:
                logger.warning(
                    f"Field id:{field_definition.field_id} could not be found in message: {message.name}"
                )
                continue
    ...
    def get_developer_fields(
        self, developer_fields_by_data_index: dict
    ) -> list[DeveloperField]:
        developer_fields = []

        for field_definition in self.developer_field_definitions:
            developer_field = developer_fields_by_data_index[
                field_definition.developer_data_index
            ][field_definition.field_id]
            if developer_field:
                sized_developer_field = DeveloperField.from_developer_field(
                    developer_field, size=field_definition.size
                )
    ...
    @classmethod
    def from_bytes(
        cls, bytes_buffer: bytes, offset: int = 0, has_developer_fields: bool = False
    ):
        # reserved
        offset += 1
        ...
        if has_developer_fields:
            ...
            developer_field_definition_size = (
                DeveloperFieldDefinition.field_definition_size()
            )
            for i in range(dev_field_count):
                fd_bytes = bytes_buffer[
                    offset : offset + developer_field_definition_size
                ]
        ...
        for field in data_message.fields:
            if field.is_valid():
                field_definitions.append(
                    FieldDefinition.from_field(field, min_string_size=min_string_size)
                )
        ...
        for field in data_message.developer_fields:
            if field.is_valid():
                developer_field_definitions.append(
                    DeveloperFieldDefinition.from_field(
                        field, min_string_size=min_string_size
                    )
                )
    ...
    def calculate_size(
        ...
        developer_field_definitions_count = (
            len(developer_field_definitions) if developer_field_definitions else 0
        )
    ...
    def supports(self, other) -> bool:
        ...
        if len(self.developer_field_definitions) != len(
            other.developer_field_definitions
        ):
            return False
